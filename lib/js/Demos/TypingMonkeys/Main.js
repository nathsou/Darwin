...
const appendChildren = (target, chilren) => {
    for (const child of chilren) {
        target.appendChild(child);
    }
    return target;
};
const span = (text) => h('span', { textContent: text });
const br = () => h('br');
const namedInput = (name, props) => {
...
searchButton.addEventListener('click', () => {
    ...
    const invalidChar = target.split('').find(char => !TypingMonkeys_1.alphabet.includes(char));
    if (invalidChar) {
