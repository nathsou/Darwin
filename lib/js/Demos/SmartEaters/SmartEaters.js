"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.SmartEaters = exports.DPR = void 0;
const CrossoverMethods_1 = require("../../CrossoverMethods");
const Darwin_1 = require("../../Darwin");
const Eater_1 = require("./Eater");
const NeuralNet_1 = require("./NeuralNet");
const Vector2D_1 = require("./Vector2D");
const MathUtils_1 = require("./MathUtils");
exports.DPR = window.devicePixelRatio;
class SmartEaters {
    constructor(cnv, params) {
        this.ticks = 0;
        this.selectedIndex = -1;
        this.followFittest = false;
        this.paused = false;
        this.fastMode = false;
        this.fastModeRefreshRate = 2;
        this.showLines = false;
        this.hideNonSelected = false;
        this.stopMating = false;
        this.cnv = cnv;
        const ctx = this.cnv.getContext('2d');
        if (ctx === null) {
            throw new Error(`Could not get a 2d canvas context`);
        }
        this.ctx = ctx;
        this.params = Object.assign({ populationSize: 30, hiddenLayersSizes: [6], foodCount: 40, crossoverRate: 0.7, mutationRate: 0.1, eliteCount: 4, eliteCopies: 1, maxSpeed: 2, maxTurnRate: 0.3, ticksPerGen: 1000, crossoverMethod: CrossoverMethods_1.CrossoverMethod.SINGLE_POINT, eaterSize: 12, foodSize: 5, wrapBorders: true }, params);
        this.layerSizes = [4, ...this.params.hiddenLayersSizes, 2];
        this.brain = new NeuralNet_1.NeuralNet();
        this.genetics = new Darwin_1.Darwin({
            populationSize: this.params.populationSize,
            randGene: () => (Math.random() * 2) - 1,
            chromosomeLength: NeuralNet_1.NeuralNet.weightsCount(this.layerSizes)
        });
        // Associate each chromosome to a Eater
        this.population = this.genetics.getPopulation().map((_, idx) => new Eater_1.Eater(this.randomPos(), Math.random() * MathUtils_1.MathUtils.TWO_PI, idx));
        this.cnv.addEventListener('click', (e) => {
            const mousePos = new Vector2D_1.Vector2D(e.clientX * exports.DPR, e.clientY * exports.DPR);
            let closestEaterIdx = 0;
            let closestEaterDist = Infinity;
            for (let i = 0; i < this.population.length; i++) {
                const distSq = this.population[i].getPosition().distSq(mousePos);
                if (distSq < closestEaterDist) {
                    closestEaterDist = distSq;
                    closestEaterIdx = i;
                }
            }
            if (Math.pow(closestEaterDist, 0.5) < this.params.eaterSize * 2 * exports.DPR) {
                this.setSelected(closestEaterIdx);
            }
            else {
                this.selectedIndex = -1;
            }
        });
        this.spawnFood();
    }
    spawnFood() {
        this.food = [];
        for (let i = 0; i < this.params.foodCount; i++) {
            this.food.push(this.randomPos());
        }
    }
    randomPos() {
        return Vector2D_1.Vector2D.rand().hadamard([this.cnv.width, this.cnv.height]);
    }
    getClosestFood(eater) {
        let closestIndex = 0;
        let closestDistSq = Infinity;
        for (let i = 0; i < this.food.length; i++) {
            const dist = eater.getPosition().distSq(this.food[i]);
            if (dist < closestDistSq) {
                closestDistSq = dist;
                closestIndex = i;
            }
        }
        return {
            index: closestIndex,
            dist: Math.pow(closestDistSq, 0.5)
        };
    }
    tick() {
        if (!this.paused) {
            if (++this.ticks > this.params.ticksPerGen) {
                this.nextGeneration();
                this.ticks = 0;
            }
            // Update positions
            for (const eater of this.population) {
                const chromo = this.genetics.getChromosomeAt(eater.getChromosomeIdx());
                this.brain.putWeights(this.layerSizes, chromo.getGenes());
                const closestFood = this.getClosestFood(eater);
                eater.closestFood = Vector2D_1.Vector2D.clone(this.food[closestFood.index]);
                // food dir
                const [foodLeft, foodRight] = this.food[closestFood.index]
                    .sub(eater.getPosition())
                    .normalize()
                    .toArray();
                // snack time!
                if (closestFood.dist < (this.params.eaterSize + this.params.foodSize) / 2) {
                    const f = chromo.getFitness();
                    chromo.setFitness(2 * (f === 0 ? 1 : f));
                    this.food[closestFood.index] = this.randomPos();
                }
                // lookat
                const lookat = [
                    Math.cos(eater.getAngle()),
                    Math.sin(eater.getAngle())
                ];
                const [turnLeft, turnRight] = this.brain.run(...lookat, foodLeft, foodRight);
                const rotationForce = MathUtils_1.MathUtils.clamp(turnLeft - turnRight, -this.params.maxTurnRate, this.params.maxTurnRate);
                eater.setAngle(eater.getAngle() + rotationForce);
                eater.lookat = new Vector2D_1.Vector2D(lookat[0], lookat[1]);
                eater.foodDir = new Vector2D_1.Vector2D(foodLeft, foodRight);
                eater.getPosition().plus(eater.lookat.times(this.params.maxSpeed * exports.DPR));
                const pos = eater.getPosition();
                if (this.params.wrapBorders) {
                    if (pos.x > this.cnv.width)
                        pos.x = 0;
                    if (pos.x < 0)
                        pos.x = this.cnv.width;
                    if (pos.y > this.cnv.height)
                        pos.y = 0;
                    if (pos.y < 0)
                        pos.y = this.cnv.height;
                }
                else {
                    if (pos.x > this.cnv.width)
                        pos.x = this.cnv.width;
                    if (pos.x < 0)
                        pos.x = 0;
                    if (pos.y > this.cnv.height)
                        pos.y = this.cnv.height;
                    if (pos.y < 0)
                        pos.y = 0;
                }
            }
        }
    }
    pause() {
        this.paused = !this.paused;
    }
    nextGeneration() {
        if (!this.stopMating) {
            this.genetics.mate();
            // reset fitness
            for (const eater of this.genetics.getPopulation()) {
                eater.setFitness(0);
            }
            this.spawnFood();
            for (const eater of this.population) {
                eater.setPosition(this.randomPos());
            }
            this.selectedIndex = -1;
        }
    }
    run() {
        const update = () => {
            this.tick();
            this.render();
            if (!this.fastMode) {
                requestAnimationFrame(update);
            }
            else {
                setTimeout(update, 0);
            }
        };
        update();
    }
    render() {
        if (!this.fastMode || this.ticks % this.fastModeRefreshRate === 0) {
            this.genetics.updateStats(true);
            this.ctx.clearRect(0, 0, this.cnv.width, this.cnv.height);
            this.drawFood();
            this.drawEaters();
            this.highlightSelectedEater();
            this.drawGenerationInfo();
        }
    }
    drawGenerationInfo() {
        const best = this.genetics.getFittest().getFitness() !== 0
            ? Math.log2(this.genetics.getFittest().getFitness())
            : 0;
        this.ctx.fillStyle = 'black';
        this.ctx.fillText(`Generation: ${this.genetics.getGeneration()}`, 5, 10);
        this.ctx.fillText(`avg: ${Math.log2(1 + this.genetics.getAverageFitness()).toFixed(3)}`, 5, 25);
        this.ctx.fillText(`best: ${best}`, 5, 40);
        this.ctx.fillText(`ticks: ${this.ticks} / ${this.params.ticksPerGen}`, 5, 55);
    }
    highlightSelectedEater() {
        if (this.followFittest) {
            this.selectedIndex = this.genetics.getStats().fittestIndex;
        }
        if (this.selectedIndex !== -1 && !this.hideNonSelected) {
            this.ctx.lineWidth = 2;
            this.ctx.strokeStyle = 'red';
            this.ctx.beginPath();
            const eater = this.population[this.selectedIndex];
            const pos = eater.getPosition();
            this.ctx.arc(pos.x / exports.DPR, pos.y / exports.DPR, this.params.eaterSize * 2, 0, MathUtils_1.MathUtils.TWO_PI);
            this.ctx.stroke();
            this.ctx.closePath();
            this.ctx.beginPath();
            this.ctx.lineWidth = 1;
            this.ctx.strokeStyle = 'black';
            this.ctx.moveTo(pos.x / exports.DPR, pos.y / exports.DPR);
            const food_dir = pos.add(eater.foodDir.times(this.params.eaterSize * 2 * exports.DPR));
            this.ctx.lineTo(food_dir.x / exports.DPR, food_dir.y / exports.DPR);
            this.ctx.stroke();
        }
    }
    drawEaters() {
        const c1 = [46, 204, 113];
        const c2 = [255, 0, 0];
        const max = Math.log2(this.genetics.getFittest().getFitness());
        let i = 0;
        for (const eater of this.population) {
            if (this.showLines) {
                this.ctx.beginPath();
                this.ctx.lineWidth = 1;
                this.ctx.strokeStyle = 'black';
                this.ctx.moveTo(eater.getPosition().x / exports.DPR, eater.getPosition().y / exports.DPR);
                const food = eater.getPosition().add(eater.foodDir.times(eater.getPosition().dist(eater.closestFood)));
                this.ctx.lineTo(food.x / exports.DPR, food.y / exports.DPR);
                this.ctx.stroke();
            }
            if (!(this.hideNonSelected && this.selectedIndex !== undefined && i++ !== this.selectedIndex)) {
                const fitness = Math.log2(this.genetics.getChromosomeAt(eater.getChromosomeIdx()).getFitness());
                const [r, g, b] = [
                    MathUtils_1.MathUtils.clamp(MathUtils_1.MathUtils.map(fitness, 0, max, c1[0], c2[0]), Math.min(c1[0], c2[0]), Math.max(c1[0], c2[0])),
                    MathUtils_1.MathUtils.clamp(MathUtils_1.MathUtils.map(fitness, 0, max, c1[1], c2[1]), Math.min(c1[1], c2[1]), Math.max(c1[1], c2[1])),
                    MathUtils_1.MathUtils.clamp(MathUtils_1.MathUtils.map(fitness, 0, max, c1[2], c2[2]), Math.min(c1[2], c2[2]), Math.max(c1[2], c2[2]))
                ];
                this.ctx.lineWidth = 4;
                this.ctx.fillStyle = `rgb(
                    ${Math.floor(r)},
                    ${Math.floor(g)},
                    ${Math.floor(b)}
                )`;
                this.ctx.beginPath();
                const la = eater.lookat.times(this.params.eaterSize * exports.DPR).add(eater.getPosition());
                const alpha = la.sub(eater.getPosition()).angle();
                const beta = Math.PI / 1.3;
                const u = new Vector2D_1.Vector2D(Math.cos(alpha + beta), Math.sin(alpha + beta));
                const v = new Vector2D_1.Vector2D(Math.cos(alpha - beta), Math.sin(alpha - beta));
                const p1 = eater.getPosition().add(u.times(this.params.eaterSize * exports.DPR));
                const p2 = eater.getPosition().add(v.times(this.params.eaterSize * exports.DPR));
                this.ctx.moveTo(p1.x / exports.DPR, p1.y / exports.DPR);
                this.ctx.lineTo(la.x / exports.DPR, la.y / exports.DPR);
                this.ctx.lineTo(p2.x / exports.DPR, p2.y / exports.DPR);
                this.ctx.fill();
            }
        }
    }
    drawFood() {
        this.ctx.fillStyle = 'rgb(52, 73, 94)';
        const fs = this.params.foodSize;
        for (const food of this.food) {
            this.ctx.beginPath();
            this.ctx.fillRect((food.x - fs / 2) / exports.DPR, (food.y - fs / 2) / exports.DPR, fs, fs);
            this.ctx.fill();
        }
    }
    setSelected(index) {
        this.followFittest = false;
        this.selectedIndex = index;
    }
    getSelected() {
        return this.population[this.selectedIndex];
    }
    getEater(idx) {
        return this.population[idx];
    }
    getEaterBrain(idx) {
        return NeuralNet_1.NeuralNet.
            fromWeights(this.layerSizes, this.genetics.getChromosomeAt(this.population[idx].getChromosomeIdx()).getGenes()).toFunction();
    }
    toggleFastMode() {
        this.fastMode = !this.fastMode;
    }
    getDarwinInstance() {
        return this.genetics;
    }
    getFittestBrain() {
        return NeuralNet_1.NeuralNet.fromWeights(this.layerSizes, this.genetics.getFittest().getGenes()).toFunction();
    }
    toggleMating() {
        this.stopMating = !this.stopMating;
    }
    getSelectedIndex() {
        return this.selectedIndex;
    }
    getParameters() {
        return this.params;
    }
}
exports.SmartEaters = SmartEaters;
