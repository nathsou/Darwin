...
class SmartEaters {
    ...
    tick() {
        ...
        for (const eater of this.population) {
            ...
            eater.position.plus(eater.lookat.times(this.params.max_speed * exports.DPR));
    ...
    drawEaters() {
        ...
        for (const eater of this.population) {
            if (this.show_lines) {
                ...
                this.ctx.lineWidth = 1;
                this.ctx.strokeStyle = 'black';
                this.ctx.moveTo(eater.position.x / exports.DPR, eater.position.y / exports.DPR);
            ...
            if (this.hide_non_selected && this.selected_idx !== undefined && i++ !== this.selected_idx)
                continue;
            ...
            this.ctx.beginPath();
            let la = eater.lookat.times(this.params.eater_size * exports.DPR).add(eater.position);
            ...
            let p1 = eater.position.add(u.times(this.params.eater_size * exports.DPR));
            let p2 = eater.position.add(v.times(this.params.eater_size * exports.DPR));
    ...
    getEaterBrain(idx) {
        return NeuralNet_1.NeuralNet.fromWeights(this.layer_sizes, this.genetics.getChromosome(this.population[idx].getChromosomeIdx()).getGenes()).toFunction();
    }
    toggleFastMode() {
    ...
    getFittestBrain() {
        return NeuralNet_1.NeuralNet.fromWeights(this.layer_sizes, this.genetics.getFittest().getGenes()).toFunction();
    }
    toggleMating() {
